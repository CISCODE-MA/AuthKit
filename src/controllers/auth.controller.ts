...
export class AuthController {
  ...
  @ApiResponse({
    status: 302,
    description: "Redirects to frontend with success/failure message.",
  })
  ...
  async verifyEmailGet(@Param("token") token: string, @Res() res: Response) {
    try {
      ...
      return res.redirect(
        `${frontendUrl}/email-verified?success=true&message=${encodeURIComponent(result.message)}`,
      );
    } catch (error) {
      ...
      return res.redirect(
        `${frontendUrl}/email-verified?success=false&message=${encodeURIComponent(errorMsg)}`,
      );
    }
  }
  ...
  @ApiBody({
    schema: { properties: { idToken: { type: "string", example: "eyJ..." } } },
  })
  ...
  async microsoftExchange(
    ...
    const { accessToken, refreshToken } = await this.oauth.loginWithMicrosoft(
      body.idToken,
    );
    return res.status(200).json({ accessToken, refreshToken });
  }
  ...
  @ApiOperation({
    summary: "Login with Google (mobile - ID token or authorization code)",
  })
  @ApiBody({
    schema: {
      properties: { idToken: { type: "string" }, code: { type: "string" } },
    },
  })
  @ApiResponse({ status: 200, description: "Login successful." })
  ...
  async googleExchange(
    @Body() body: { idToken?: string; code?: string },
    @Res() res: Response,
  ) {
  ...
  @ApiBody({
    schema: {
      properties: { accessToken: { type: "string", example: "EAABw..." } },
    },
  })
  @ApiResponse({ status: 200, description: "Login successful." })
  ...
  googleCallback(
    ...
    passport.authenticate(
      ...
      (err: any, data: any) => {
        ...
        return res.redirect(
          `${frontendUrl}/oauth-callback?accessToken=${accessToken}&refreshToken=${refreshToken}&provider=google`,
        );
      },
    )(req, res, next);
  }
  ...
  @ApiOperation({
    summary: "Initiate Microsoft OAuth login (web redirect flow)",
  })
  @ApiResponse({
    status: 302,
      ...
      (err: any, data: any) => {
        if (err || !data) {
          const frontendUrl =
            process.env.FRONTEND_URL || "http://localhost:5173";
          return res.redirect(
            `${frontendUrl}/login?error=microsoft_auth_failed`,
          );
        }
        const { accessToken, refreshToken } = data;
        const frontendUrl = process.env.FRONTEND_URL || "http://localhost:5173";
        return res.redirect(
          `${frontendUrl}/oauth-callback?accessToken=${accessToken}&refreshToken=${refreshToken}&provider=microsoft`,
        );
      },
    )(req, res, next);
  }
  ...
  @ApiOperation({
    summary: "Initiate Facebook OAuth login (web redirect flow)",
  })
  @ApiResponse({
    status: 302,
      ...
      (err: any, data: any) => {
        if (err || !data) {
          const frontendUrl =
            process.env.FRONTEND_URL || "http://localhost:5173";
          return res.redirect(
            `${frontendUrl}/login?error=facebook_auth_failed`,
          );
        }
        const { accessToken, refreshToken } = data;
        const frontendUrl = process.env.FRONTEND_URL || "http://localhost:5173";
        return res.redirect(
          `${frontendUrl}/oauth-callback?accessToken=${accessToken}&refreshToken=${refreshToken}&provider=facebook`,
        );
      },
    )(req, res, next);
  }
