...
export class OAuthService {
  ...
  async loginWithMicrosoft(idToken: string): Promise<OAuthTokens> {
    const profile =
      await this.microsoftProvider.verifyAndExtractProfile(idToken);
    return this.findOrCreateOAuthUserFromProfile(profile);
  }
  ...
  async loginWithFacebook(accessToken: string): Promise<OAuthTokens> {
    const profile =
      await this.facebookProvider.verifyAndExtractProfile(accessToken);
    return this.findOrCreateOAuthUserFromProfile(profile);
  }
   ...
   * @returns Authentication tokens for the user
   */
  private async findOrCreateOAuthUserFromProfile(
   ...
   */
  private async handleDuplicateUserCreation(
    email: string,
  ): Promise<OAuthTokens> {
    try {
      ...
      if (user) {
        const { accessToken, refreshToken } =
          await this.auth.issueTokensForUser(user._id.toString());
        return { accessToken, refreshToken };
      }
    } catch (retryError) {
      this.logger.error(
